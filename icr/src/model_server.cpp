...
void ModelServer::getModelStates(gazebo_msgs::ModelStates const & states)
{
  ...
  for(icr_object_id=0; icr_object_id < states.name.size(); icr_object_id++)
    {
    if(states.name[icr_object_id]==root_link_name_)
      {
	obj_spawned=true;
	break;
      }
  ...
  static tf::TransformBroadcaster tf_brc;
  ...
  tf_brc.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", root_link_name_));
...
bool ModelServer::loadModel(icr::load_model::Request  &req, icr::load_model::Response &res)
{
  ...
  delete_model.request.model_name=root_link_name_;
  ...
  if(!gazebo_delete_clt_.call(delete_model))
    ROS_WARN("Could not delete previous icr_object model %s",root_link_name_.c_str());
  ...
  if(!urdf_model.initString(model))
    {
      ROS_ERROR("Could not parse urdf model %s",req.urdf_file.c_str());
      data_mutex_.unlock();
      return res.success;
    }
  root_link_name_=urdf_model.getRoot()->name;

  gazebo_msgs::SpawnModel spawn_model;
  spawn_model.request.model_name=root_link_name_; //name the model after its base link
